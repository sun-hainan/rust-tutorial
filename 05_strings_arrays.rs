...
//   - slice 是动态数组（类似于 Vec），array 是固定长度
//   - 字符串索引按字节（rune 索引需转换）
//   - sort 包提供排序，sort.Search 二分查找
//   - regexp 提供正则表达式

// C++:
//   - std::string 可变字符串
//   - std::vector 是动态数组，std::array 是固定长度
//   - 字符串按 char 字节索引（UTF-8 需处理）
//   - std::sort 原地排序，std::binary_search 二分查找
//   - <regex> 提供正则表达式
